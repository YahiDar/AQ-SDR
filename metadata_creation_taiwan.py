...
gdf = gpd.read_file('./utils/taiwan.geojson')
...
def group_stations_by_grid(full_metadata, full_grids, radius_m = 2000, same_country_limit = True, store_to_json = None):
    ...
    if store_to_json != None:
        os.makedirs(os.path.dirname(store_to_json),exist_ok=True)
...
def sorted_by_filtered_stations(data_dict, TW_ONLY=False):
    """
    ...
    Args:
        ...
        TW_ONLY (bool): If True, keep only Taiwan data
    ...
    # Filter for Taiwan only if specified
    if TW_ONLY:
        if 'Taiwan' in data_dict:
            working_dict = {'Taiwan': data_dict['Taiwan']}
        else:
            return {}
    else:
            ...
            for station in stations_list[1]:
                ...
                if filtered_stations:
                    if any(isinstance(item[0], str) and
                        ...
                        continue
    if sort:
        return sorted_by_filtered_stations(processed_dict, TW_ONLY = TW_ONLY)
    else:
...
def dataloader(searcher_output,
    ...
    Inputs:
        ...
        it is a dictionary where the key is region names (e.g. 'Taiwan'), and values is a list of station id as a string (e.g. ['UTR_bu011','UTR_bu008'])
        ...
        - return_multiple_df: return a separate df for every station (as a dictionary)
            ...
            If there are multiple regions, then this will result in a nested dictionary (e.g.: {'Taiwan':{'station_1': dataframe, 'station_2': dataframe2 ...}})
...
def find_sensors_in_radius(full_meta, 
    ...
    TW_ONLY : bool
        search only for sensors in the Taiwan
    return_as_dict : bool
        return as dictionary: {'Taiwan': [list]}
    ...
    if return_as_dict:
        return {'Taiwan': matching_sensors}
